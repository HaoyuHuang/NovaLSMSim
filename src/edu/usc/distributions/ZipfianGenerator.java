...
public class ZipfianGenerator extends NumberGenerator {
	...
	public ZipfianGenerator(long min, long max, double zipfianconstant,
		...
		this(min, max, zipfianconstant,
				zetastatic(max - min + 1, zipfianconstant), rand);
	}
	 ...
	 * thetaVal. Remember the value of n, so if we change the itemcount, we can
	 * recompute zeta.
	 *
	...
	public long nextLong(long itemcount) {
		...
		if (itemcount != countforzeta) {
			...
			if (itemcount > countforzeta) {
				...
				// we have added more items. can compute zetan incrementally,
				// which is cheaper
				...
				// TODO: can also have a negative incremental computation, e.g.
				// if you decrease
				// the number of items,
				// then just subtract the zeta sequence terms for the items that
				// went away. This
				...
				System.err.println(
						"WARNING: Recomputing Zipfian distribtion. This is slow and should be avoided. "
								+ "(itemcount=" + itemcount + " countforzeta="
								+ countforzeta + ")");
	...
	public static double zipfCdfApprox(double k, double s, double N) {
		...
		double a = (Math.pow(k, 1 - s) - 1) / (1 - s) + 0.5
				+ Math.pow(k, -s) / 2 + s / 12 - Math.pow(k, -1 - s) * s / 12;
		double b = (Math.pow(N, 1 - s) - 1) / (1 - s) + 0.5
				+ Math.pow(N, -s) / 2 + s / 12 - Math.pow(N, -1 - s) * s / 12;
	...
	public static void main(String[] args) {
		int n = 10000000;
		for (double alpha : new double[] { 0.99 }) {
			ZipfianGenerator gen = new ZipfianGenerator(n, alpha, new Random());
			int cnt = 0;
			int total = 10000000;
			System.out.println(n / 10);
			for (int i = 0; i < total; i++) {
//				if (gen.nextValue().longValue() < n / 1000) {
//					cnt++;
//				}
				if (gen.nextValue().longValue() == 0) {
					cnt++;
				}
			}
			System.out.println(cnt);
			System.out.println(total);
			System.out.println((double)cnt / total);
		}
	}
}
