...
public class ValueSizeGenerator {
	...
	public ValueSizeGenerator(Random rand) {
		...
		this.valueSizeDist = new GeneralizedParetoDistribution(loc, scale,
				shape);
		// 1 MB.
		int maxSize = 1000001;
	...
	public int get() {
		int val = Utility.select(this.valueSizeCDF, rand.nextDouble(),
				(int) this.valueSizeDist.getMean(),
				this.valueSizeCDF.length - 1);
		return val;
	}

	public int getCDF(double p) {
		int maxSize = 1000001;
		for (int i = 1; i < maxSize; i++) {
			if (this.valueSizeCDF[i] >= p) {
				return i;
			}
		}
		return maxSize - 1;
	...
	public static void main(String[] args) {
		ValueSizeGenerator valueSizeGenerator = new ValueSizeGenerator(
				new Random());
		System.out.println(valueSizeGenerator.getValueSizeDist().getMean());
		KeySizeGenerator keySizeGenerator = new KeySizeGenerator(new Random());
		System.out.println(keySizeGenerator.getKeySizeDist().getMean());
	}
}
