...
public class KeySizeGenerator {
	...
	public KeySizeGenerator(Random rand) {
		...
		this.keySizeDist = new GeneralizedExtremeValueDistribution(loc, scale,
				shape);
		int maxKeySize = 251;
	...
	public int get() {
		return Utility.select(this.keySizeCDF, rand.nextDouble(),
				(int) this.keySizeDist.getMean(), this.keySizeCDF.length - 1);
	}

	public int getCDF(double p) {
		int maxKeySize = 251;
		for (int i = 1; i < maxKeySize; i++) {
			if (this.keySizeCDF[i] >= p) {
				return i;
			}
		}
		return maxKeySize - 1;
